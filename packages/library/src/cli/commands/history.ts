...
/**
 * Handler for the `trackgentic history <issueId>` command.
 */
export async function historyAction(issueId: string): Promise<void> {
  try {
    const tracker = new Tracker();
    const result = await tracker.history(issueId);
    writeStdout(result);
    process.exit(0);
  } catch (err) {
