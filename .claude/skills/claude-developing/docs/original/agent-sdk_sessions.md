...
<Note>
  Sessions persist the **conversation**, not the filesystem. To snapshot and revert file changes the agent made, use [file checkpointing](agent-sdk_file-checkpointing.md).
</Note>
...
How much session handling you need depends on your application's shape. Session management comes into play when you send multiple prompts that should share context. Within a single `query()` call, the agent already takes as many turns as it needs, and permission prompts and `AskUserQuestion` are [handled in-loop](agent-sdk_user-input.md) (they don't end the call).
...
| Stateless task, don't want anything written to disk (TypeScript only) | Set [`persistSession: false`](agent-sdk_typescript.md#options). The session exists only in memory for the duration of the call. Python always persists to disk. |

### Continue, resume, and fork

Continue, resume, and fork are option fields you set on `query()` ([`ClaudeAgentOptions`](agent-sdk_python.md#claude-agent-options) in Python, [`Options`](agent-sdk_typescript.md#options) in TypeScript).
...
[`ClaudeSDKClient`](agent-sdk_python.md#claude-sdk-client) handles session IDs internally. Each call to `client.query()` automatically continues the same session. Call [`client.receive_response()`](agent-sdk_python.md#claude-sdk-client) to iterate over the messages for the current query. The client must be used as an async context manager.
...
See the [Python SDK reference](agent-sdk_python.md#choosing-between-query-and-claude-sdk-client) for details on when to use `ClaudeSDKClient` vs the standalone `query()` function.
...
<Note>
  There's also a [V2 preview](agent-sdk_typescript-v2-preview.md) of the TypeScript SDK that provides `createSession()` with a `send` / `stream` pattern, closer to Python's `ClaudeSDKClient` in feel. V2 is unstable and its APIs may change; the rest of this documentation uses the stable V1 `query()` function.
</Note>
...
* **Recover from a limit.** The first run ended with `error_max_turns` or `error_max_budget_usd` (see [Handle the result](agent-sdk_agent-loop.md#handle-the-result)); resume with a higher limit.
...
To resume sessions across machines or in serverless environments, mirror transcripts to shared storage with a [`SessionStore` adapter](agent-sdk_session-storage.md).
...
<Note>
  Forking branches the conversation history, not the filesystem. If a forked agent edits files, those changes are real and visible to any session working in the same directory. To branch and revert file changes, use [file checkpointing](agent-sdk_file-checkpointing.md).
</Note>
...
Both SDKs expose functions for enumerating sessions on disk and reading their messages: [`listSessions()`](agent-sdk_typescript.md#list-sessions) and [`getSessionMessages()`](agent-sdk_typescript.md#get-session-messages) in TypeScript, [`list_sessions()`](agent-sdk_python.md#list-sessions) and [`get_session_messages()`](agent-sdk_python.md#get-session-messages) in Python. Use them to build custom session pickers, cleanup logic, or transcript viewers.

Both SDKs also expose functions for looking up and mutating individual sessions: [`get_session_info()`](agent-sdk_python.md#get-session-info), [`rename_session()`](agent-sdk_python.md#rename-session), and [`tag_session()`](agent-sdk_python.md#tag-session) in Python, and [`getSessionInfo()`](agent-sdk_typescript.md#get-session-info), [`renameSession()`](agent-sdk_typescript.md#rename-session), and [`tagSession()`](agent-sdk_typescript.md#tag-session) in TypeScript. Use them to organize sessions by tag or give them human-readable titles.

## Related resources

* [How the agent loop works](agent-sdk_agent-loop.md): Understand turns, messages, and context accumulation within a session
* [File checkpointing](agent-sdk_file-checkpointing.md): Track and revert file changes across sessions
* [Python `ClaudeAgentOptions`](agent-sdk_python.md#claude-agent-options): Full session option reference for Python
* [TypeScript `Options`](agent-sdk_typescript.md#options): Full session option reference for TypeScript
