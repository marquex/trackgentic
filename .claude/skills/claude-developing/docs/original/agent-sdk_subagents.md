...
* **Programmatically**: use the `agents` parameter in your `query()` options ([TypeScript](agent-sdk_typescript.md#agent-definition), [Python](agent-sdk_python.md#agent-definition))
...
In the Python SDK, these field names use camelCase to match the wire format. See the [`AgentDefinition` reference](agent-sdk_python.md#agent-definition) for details.
...
* [SDK overview](agent-sdk_overview.md): getting started with the Claude Agent SDK
